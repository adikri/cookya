...
### Android distribution via EAS Build
**Decision:** Use Expo Application Services (EAS) Build to produce signed Android binaries. Path 1 (`preview` profile → APK) for partner and personal testing now; Path 2 (`production` profile → AAB → Play Console internal testing track) when graduating to managed distribution; Path 3 (Play Store production track) when going public.
**Options considered:** Manual local builds via `expo run:android` + Android SDK + signing config (free but every dev needs the setup); Firebase App Distribution (free, beta-only, doesn't graduate to Play); EAS Build cloud (free tier 30 builds/month, $99/yr unlimited); ad-hoc APK sharing without managed signing (keystore loss risk).
**Reason:** EAS is the official Expo recommendation, requires no local Android SDK setup, manages keystore generation and storage (with `eas credentials` backup escape hatch), produces signed APKs partner can install in one tap, and the same pipeline graduates cleanly to Play Store AABs by changing one profile in `eas.json`. Free tier is sufficient for the solo+partner workflow; the $25 Play Console fee one-time is the only paid cost. This maps the Apple Xcode → installed app workflow Adi already knows: "build once, app stays on phone." Implementation tracked as slice 1 in WORKLOG carry-forward (`infra/eas-android-distribution` branch).
